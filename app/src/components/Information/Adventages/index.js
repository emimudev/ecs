...
function Properties({ type = 'Adventages' }) {
      ...
      <>
        {adventages.map(({ title, description, icon }, i) => (
          <Box p={4} rounded='md' key={`highlight_${i}`}>
            ...
            <Text fontWeight={500} fontSize='lg' color='#f687b3'>{title}</Text>
    ...
    return (
      <>
        {weakness.map(({ title, description, icon }, i) => (
          <Box p={4} rounded='md' key={`highlight_${i}`}>
            <Text fontSize='4xl'>{icon}</Text>
            <Text fontWeight={500} color='#f687b3' fontSize='lg'>{title}</Text>
            <Text color={color} mt={4} textAlign='left'>
              {description}
            </Text>
          </Box>
  ...
  return (
    <Container maxW='container.xl' centerContent gap='10'>
      {(type === 'Adventages') && <AdventagesTitle />}
      {(type === 'Weakness') && <WeaknessTitle />}
      <SimpleGrid spacingX={10} minChildWidth='300px'>
        {(type === 'Adventages') && <Adventages />}
